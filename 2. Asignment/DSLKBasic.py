class Date:
    ...
    def toStringNgay(self):
        """Store day as string format dd/mm/yy"""
...
def addSV(head, std: SinhVien):
    """Add a student to a sorted list of student (in increasing order of ID) so that it is still sorted"""
    temp = Node()
    current = temp
    element = Node(std)
...
def createListSV():
    """Create a sorted list of student with input from terminal"""
    ...
    for i in range(n):
        ...
        s_class = input("Lop: ")
        major = input("Khoa: ")
        std = SinhVien(ID, name, sex, birth, address, s_class, major)
        head = addSV(head, std)
    return head
